...
var worker_default = {
  async fetch(request, env2, ctx) {
    ...
    try {
      if (url.pathname.startsWith("/_next/") || url.pathname.includes(".") || url.pathname === "/favicon.ico") {
        return env2.ASSETS.fetch(request);
      }
      if (url.pathname === "/") {
        return env2.ASSETS.fetch(new Request(url.origin + "/index.html"));
      }
      let response = await env2.ASSETS.fetch(request);
      if (response.status === 404) {
        const htmlPath = url.pathname.endsWith("/") ? url.pathname + "index.html" : url.pathname + ".html";
        response = await env2.ASSETS.fetch(new Request(url.origin + htmlPath));
        if (response.status === 404) {
          const indexPath = url.pathname.endsWith("/") ? url.pathname + "index.html" : url.pathname + "/index.html";
          response = await env2.ASSETS.fetch(new Request(url.origin + indexPath));
        }
      }
      return response;
    } catch (error3) {
...
// .wrangler/tmp/bundle-gbzOuk/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gbzOuk/middleware-loader.entry.ts
